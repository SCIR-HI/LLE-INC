...
def barycenter_weights(X, Y, indices, reg=1e-3):
    ...
    # note that X==Y in our case
    indices = check_array(indices, dtype=int)
    # the k nearest neighbors for points in X

    n_samples, n_neighbors = indices.shape
    assert X.shape[0] == n_samples

    B = np.empty((n_samples, n_neighbors), dtype=X.dtype)
    # B s_samples x n_neighbors Empty array
    v = np.ones(n_neighbors, dtype=X.dtype)
    # v n_neighbors One vector

    # this might raise a LinalgError if G is singular and has trace
    # zero
    for i, ind in enumerate(indices):
        # i: index, ind: index of n_neighbors
        A = Y[ind]
        # A: n neighbors of X
...
def barycenter_kneighbors_graph(X,  X_label, n_neighbors, reg=1e-3, n_jobs=None):
    ...
    """
    # construct a dict: {key(label_id):value[index of samples]}
    label_dict = {}
    for label_index in range(len(X_label)):
        label_dict.setdefault(X_label[label_index], [])
        label_dict[X_label[label_index]].append(label_index)

    # knn = NearestNeighbors(n_neighbors=n_neighbors + 1, n_jobs=n_jobs).fit(X)
    knn = NearestNeighbors(n_neighbors=len(X_label), n_jobs=n_jobs).fit(X)
    X = knn._fit_X
    n_samples = knn.n_samples_fit_
    assert n_samples == len(X_label)
    ind = knn.kneighbors(X, return_distance=False)[:, 1:]
    # Attention: modify the ``ind'' with the index of neighbors with the same classification label
    # strategy:
    #   1. select the top k nearest neighbors with the same classification label
    #   2. randomly select k samples with the same classification label
    #   3.
    # ind: index of k nearest neighbors of X (n_samples x k)

    # strategy 1
    ind_ls = ind.tolist()
    new_ind_ls = []
    for l in range(len(ind_ls)):
        l_knn = []
        for m in ind_ls[l]:
            if m in label_dict[X_label[l]]:
                l_knn.append(m)
                if len(l_knn) == n_neighbors:
                    break
        new_ind_ls.append(l_knn)


    # pdb.set_trace()
    ind = np.array(new_ind_ls)
    data = barycenter_weights(X, X, ind, reg=reg)
    # data: B in barycenter (n_samples x n_neighbors)
...
def locally_linear_embedding_mod(
    X,
    X_label,
    *,
    n_neighbors,
    ...
    if method == "standard":
        W = barycenter_kneighbors_graph(
            nbrs, X_label=X_label, n_neighbors=n_neighbors, reg=reg, n_jobs=n_jobs
        )
...
class LocallyLinearEmbeddingMod(TransformerMixin, _UnstableArchMixin, BaseEstimator):
    ...
    def _fit_transform(self, X):
        ...
        self.nbrs_.fit(X)
        self.embedding_, self.reconstruction_error_ = locally_linear_embedding_mod(
            X=self.nbrs_,
